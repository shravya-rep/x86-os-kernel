...
addr_perm(struct proc *p, const void *vaddr, int perm)
{
        ...
        vmarea_t *vma;
        if((vma=vmmap_lookup(map,ADDR_TO_PN(vaddr)))==NULL)
        {
            dbg(DBG_PRINT, "(GRADING3D 2)\n");
            dbg(DBG_PRINT, "(GRADING3D 1)\n");
            return 0;
        }
        if(vma->vma_prot&perm)
        {
            dbg(DBG_PRINT, "(GRADING3D 2)\n");
            dbg(DBG_PRINT, "(GRADING3D 1)\n");
            return 1;
        ...
        list_iterate_begin(&(map->vmm_list), vma, vmarea_t, vma_olink) {
            if((vaddr < PN_TO_ADDR(vma->vma_end)) && (vaddr >= PN_TO_ADDR(vma->vma_start))){
                //PROT_READ 0x1, PROT_WRITE 0x2, PROT_EXEC 0x4
                if ((vma->vma_prot & perm) == perm){
                    return 1;
                }
                else {
                    return 0;
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
        ...
        vaddr = (uint32_t)PAGE_ALIGN_DOWN(avaddr);
        
        while(vaddr < ((uint32_t)avaddr + len)){
            retVal = addr_perm(p, (void *)vaddr, perm);
            if (retVal == 0){
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                return 0;
            }
