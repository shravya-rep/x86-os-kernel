...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
    ...
    while(1) {
    pframe_t* retVal=pframe_get_resident(o,pagenum);
    if(!retVal)
    {
        retVal=pframe_alloc(o,pagenum);
        if(!retVal) {
            dbg(DBG_PRINT, "(GRADING3A)\n");
            return -ENOMEM;
        }
        int returnValue= pframe_fill(retVal);
        if(returnValue!=0)
        {
            pframe_free(retVal);
            dbg(DBG_PRINT, "(GRADING3A)\n");
    ...
    if(pframe_is_busy(retVal))
    {

        sched_sleep_on(&retVal->pf_waitq);
        dbg(DBG_PRINT, "(GRADING3A)\n");
    }
    *result=&retVal[0];
    KASSERT(NULL != *result);
