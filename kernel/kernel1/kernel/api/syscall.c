...
sys_read(read_args_t *arg)
{
        ...
        //Retrieving the values of the arguments. (Is this necessary?)
        ...
        //Allocating a page to temporarily hold the data.
        buf = page_alloc();
        /*
        if (buf == NULL) {
        ...
        //Calling do_read to pull the data from the file descriptor.
        ret = do_read(kern_args.fd, buf, kern_args.nbytes);
        //If the read was successful, put that data into the user space at the specified location
        if (ret > 0) {
                err = copy_to_user(kern_args.buf, buf, kern_args.nbytes);
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
        }
...
sys_write(write_args_t *arg)
{
        ...
        //Retrieving the buffer values from the user space (I think some things from here on out are wrong).
        copy_from_user(buf,kern_args.buf,kern_args.nbytes);
        
        //Write the user space version to the file.
        ret = do_write(kern_args.fd, buf, kern_args.nbytes);

        //If it was successful, why are we copying back to the user space? 
        /*
        if (ret > 0) {
                err = copy_to_user(kern_args.buf, buf, strlen(buf));
        }
        */
...
sys_getdents(getdents_args_t *arg)
{
        ...
        //int fd;
        dirent_t *dirp = kmalloc(sizeof(dirent_t));
        //size_t count;
        int ret;
        size_t totalRet = 0;
        int err;
        uintptr_t currentWriteAddress;
        ...
        //Set up the while loop to call getdents the right number of times
        size_t amountToCopy;
        while(totalRet < kern_args.count){
            ret = do_getdent(kern_args.fd, dirp);
            if (ret < 0) {
                curthr->kt_errno = -ret;
                kfree(dirp);
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
            ...
            amountToCopy = MIN(sizeof(dirent_t), kern_args.count - totalRet);
            if(amountToCopy < sizeof(dirent_t) || ret == 0) {
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                break;}
            currentWriteAddress = (uintptr_t)(kern_args.dirp) + totalRet;

          if ((err = copy_to_user((void *)currentWriteAddress, dirp, amountToCopy)) < 0) {
                curthr->kt_errno = -err;
                kfree(dirp);
                dbg(DBG_PRINT, "(GRADING3D 2)\n");
                return -1;
