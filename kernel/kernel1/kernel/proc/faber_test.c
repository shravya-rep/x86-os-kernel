...
void *faber_thread_test(int arg1, void *arg2) {
    ...
    dbg(DBG_TEST, "\n>>> Start running faber_thread_test()...\n");

#if CS402TESTS > 0
    dbg(DBG_TEST, "(C.1) start\n");
...
#if CS402TESTS > 8
    dbg(DBG_TEST, "(C.9) start (no end)\n");
    dbg(DBG_TEST, "proc kill all test\n");
    KASSERT(curproc && curproc->p_pid != PID_INIT && "Error: running (C.9) from INIT is not permitted");
    for ( i=0 ; i < 10; i++ )
